...
function drawHands() {
    ...
    if (TwoOne.jogoStatus == 0) {
        TwoOne.caixaOptext.innerHTML = "Mão do oponente (" + TwoOne.maoOponente[1].value + ")"; // hide value while a card is face down
    } else {
...
TwoOne.passar.addEventListener("click", function passarLoop() {
    ...
    if (TwoOne.jogoStatus === 2) {
        return;
    } else if (TwoOne.jogoStatus === 0) // i.e. Passar foi apenas pressionado
    {
...
function vitoria() {
    ...
    TwoOne.caixaDeTexto.innerHTML = "Você ganhou!!! BORA TOMAR UMA? 🤢 <br>" + explanation + "<br>pressione 'Novo Jogo' para beber... ops jogar novamente.";
    track();
}
var empate = function() {
    TwoOne.games += 1;
    ...
    TwoOne.textUpdates.innerHTML = "Empate! SEM BEBIDA PRA NINGUÉM <br>Pressione 'Novo Jogo' para jogar novamente.";
}
...
function derrota() {
    ...
    TwoOne.caixaDeTexto.innerHTML = "Você perdeu 🤮 SEM VITÓRIA SEM BEBIDA! <br>" + explanation + "<br>Pressione 'Novo Jogo' para beber... ops jogar novamente.";
    track();
}
